...
namespace MimicSpace
{
    public class Mimic : MonoBehaviour
    {
        ...
        public float attackingRange = 10f;
        public Transform legAttackPoint;
        public GameObject attackPrefab;
        public LayerMask enemyLayerMask;// If you have a specific prefab for attack
        ...
        {
            if (legAttackPoint == null) // if no specific attack point is designated, use the enemy's position
                legAttackPoint = enemy.transform;

         
            RequestLeg(legAttackPoint.position, legResolution, maxLegDistance, maxGrowCoef, this, 2f); // Here you can set the lifetime to be short for attack
        ...
        void Update()
        {
            ...
            Collider[] hitEnemies = Physics.OverlapSphere(transform.position, attackingRange,enemyLayerMask);
            if (hitEnemies.Length > 0)
            {
                foreach (var hitEnemy in hitEnemies)
                {
                   
                        AttackEnemy(hitEnemy.gameObject);
                        break;
                }
            }
        }
